...
const Options = () => {
  ...
  const { adPurpose, selectedGender } = useTextareaContext(); // Use context
  ...
  const handlePreviewClick = () => {
    if (!adPurpose || adPurpose.size === 0 || !selectedGender || selectedGender.size === 0) {
      setShowAlert(true);
      setTimeout(() => {
        setShowAlert(false);
      }, 1000);
  ...
  return (
    <div style={{ height: "530px", width: "640px", overflow: "hidden" }} className="z-index-1">
      ...
      <div className="relative isolate px-6 pt-14 lg:px-8 z-index-1">
        ...
        <div className="flex flex-col items-center justify-center" style={{marginLeft: '7px'}}>
        <AgeSelector/>
        <Spacer x={4} />
        <Spacer x={4} />
        <Spacer x={4} />
